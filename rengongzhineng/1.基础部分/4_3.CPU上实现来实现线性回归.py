...
# 1111111111111.定义device对象___判断是否cuda是否可用，如果可用传入cuda字符串
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# 1.准备数据
# 222222222222.将输入数据转为device类型
...
# 3333333333333333.将模型也转为device
my_linear = myLr().to(device)  # 实例化模型
optimizer = optim.SGD(my_linear.parameters(), 0.01)  # 实例化优化器
...
# my_linear.eval()  # 设置评估模式，精啊my_linear.train()转为false
# predict = my_linear(x)
